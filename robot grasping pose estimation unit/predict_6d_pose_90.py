...
def get_rot_loss(robot_trajectory, robot_rot):
	...
	for i in range(BATCH_SIZE):
		...
		loss = np.minimum(np.abs(90.0 - loss), loss)
		rot_loss[i,-1] = loss
...
with tf.Session(config=config) as session:
	...
	x = np.linspace(-45, 44, 90, endpoint=True)
	y = rot_loss

	data = pd.DataFrame(np.random.randn(90,2), columns=['trans_loss','rot_loss'])
	data['trans_loss'] = trans_loss
	data['rot_loss'] = rot_loss
	data.to_csv('loss_90.csv')

	print('90 degrees trans_loss_mean:', trans_loss_mean)
	print('90 degrees rot_loss_mean:', rot_loss_mean)
	...
	plt.title('90 degrees trans_loss')
	plt.subplot(212)
